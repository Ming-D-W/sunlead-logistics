  ...
  <div class="order-manage">
    <el-card class="top-card">
      <el-form>
        <el-row>
          <el-col :span="8">
            <el-form-item label="派件任务编号:" label-width="100px">
              <el-input v-model="searchText.id" placeholder="请输入派件任务编号"/>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="订单编号:" label-width="100px">
              <el-input v-model="searchText.orderId" placeholder="请输入派件订单编号"/>
            </el-form-item>
          </el-col>
          <el-col :span="8">
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="签收状态:" label-width="100px">
              <el-select v-model="searchText.signStatus" placeholder="请选择" clearable>
                <el-option v-for="item in signStatusOptions" :key="item.value" :label="item.label"
                           :value="item.value"/>
              </el-select>
            </el-form-item>
          </el-col>
    ...
    <el-card style="margin-top: 20px">
      ...
      <el-table :row-style="{fontSize:'13px'}" v-loading="loading" :data="tableData"
        ...
        <el-table-column label="派件作业编号" prop="id" width="180px"/>
        <el-table-column label="订单编号" prop="orderId" width="180px"/>
        ...
        <el-table-column label="签收状态">
          <template #default="{row}">
            {{ row.signStatus === 1 ? '已签收' : '拒收' }}
          </template>
        </el-table-column>
        <el-table-column label="签收人">
          <template #default="{row}">
            {{ row.signRecipient === 1 ? '本人' : '他人' }}
          </template>
        </el-table-column>
...
import { getDispatchList } from '@/api/transport'

export default {
  name: 'dispatchTask-page',
